...
from automated_clinical_coding_llm.tree_search_icd import get_icd_codes
...
GEMINI_MODEL_NAME = "gemini-2.0-flash" # LLM mdoel name
...
if __name__ == "__main__":
    ...
    output_file = f'data/output/experiment_tree_search/{DATASET}/{LANG}/icd_predictions.json'
    ...
    for report in tqdm(reports, desc="Processing medical reports"):
        ...
        # Run tree search pipeline to predict ICD codes
        icd_codes = get_icd_codes(report_text, GEMINI_MODEL_NAME, temperature=LLM_TEMPERATURE)
        code_map[report_id] = icd_codes

        # Save the predicted ICD codes to a JSON file
    ...
    # Run evaluation
    run_evaluation(DATASET, 'tree_search', LANG)