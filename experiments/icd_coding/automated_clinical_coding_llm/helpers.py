...
from automated_clinical_coding_llm.prompt_templates import *
...
def build_translation_prompt(input_note, system_prompt=""):
    ...
    messages = []
    if system_prompt:
        # In Gemini, system instructions are often handled implicitly or as part of the initial prompt.
        # For a more direct translation, you might include it as the first part of the user's turn.
        # Or, if using a chat session, it would be a 'model' role setting context.
        # For direct 'generate_content', we'll treat it as part of the user's initial input.
        messages.append({"role": "user", "parts": [system_prompt]})
        messages.append({"role": "model", "parts": ["Understood. I will now perform the translation."]}) # Acknowledge system prompt
    messages.append({"role": "user", "parts": [input_prompt]})
    return messages
...
def build_zero_shot_prompt(input_note, descriptions, model_name, system_prompt=""):
    ...
    messages = []
    if system_prompt:
        # System prompts can be part of the initial chat history or integrated into the user prompt.
        # For simplicity, we'll add it as a user message followed by a model acknowledgment,
        # mimicking a chat conversation for Gemini.
        messages.append({"role": "user", "parts": [system_prompt]})
        messages.append({"role": "model", "parts": ["Understood. I will proceed with the classification."]})
